...
"""
	...
	这个模块是找出m.baojia.com里面的所有链接，包括所有的ajax请求的链接，
	然后对每一个链接做单元测试，查看是否有漏洞存在

	:copyright: baojia.com 2015
...
# 这个参数是需要查找的URL地址
__target_url='http://m.baojia.com'
#记录下要抓取网站的长度
...
#需要忽略的URL
__ignore_dict=('＃','javascript:;','javascript:','javascript','javascript:void(0)','javascript:void()','tel:400-010-0666')
...
def spider_start_work():
	...
	while __url_to_deal_queue.empty() == False:
...
#抓取页面方法
def find_target_url(__current_url):
	#logging.info('find target url: %s ' % target_url_)
	with request.urlopen(__current_url,) as page_data:
		server_status=page_data.status
		server_reason=page_data.reason
		if server_status!=200 or server_reason!='OK':
			logging.info('targer url [%s]unreachable server_status is %s server_reason is %s' % (__current_url,server_status,server_reason))
			pass
		else:
			current_page_data=page_data.read()
			if __current_url[-3:] =='.js':
...
def find_current_page_js(current_page_data):
	...
	list=cond.findall(current_page_data)
	for i in list:
		current_url=i[6:]
...
def add_to_queue(current_url):
	global __url_to_deal_queue
	if is_url_need_to_record(current_url) == True:
		if current_url[0]=="/":
